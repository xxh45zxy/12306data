...
如果你熟悉Python操作，**可以直接用`requirements.txt`安装依赖**，之后阅读[运行脚本](#25-运行脚本)部分。
...
```bash
pip install numpy==1.24.2 pandas==2.0.2 selenium==4.24.0 webdriver-manager==4.0.2 webdrivermanager_cn==2.4.1
```

如果你希望以后运行更多的Python项目，你可能需要进行依赖管理，具体请搜索`venv`或`Anaconda`有关教程。
...
**如果脚本提示需选择，按实际情况输入字符选择**，详见[注意事项](#3-注意事项)第3点。

脚本提示`Press Enter to continue...`时，回车，关闭脚本。

#### 2.5.4 查看结果

回到脚本所在目录，查看以`.csv`文件保存的结果。这些文件可在Excel或WPS中打开。

关于不同文件涉及的数据范围，请参考[脚本支持保存的数据范围](#1-脚本支持保存的数据范围)。
...
- 由于12306仅保留近30天数据，建议**至少每25天[运行一次本脚本](#25-运行脚本)**。
