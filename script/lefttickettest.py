...
class SearchTicket:
    ...
    def mobile_search(self) -> dict:
        ...
        headers['Cookie'] = get_cookie()
        api = 'com.cars.otsmobile.queryLeftTicketR'
        '''data = [{"train_date": f"{self.date}", "purpose_codes": "00",
                 ...
                 "baseDTO": {"check_code": f"{random_string(32)}",
                             ...
                             "h5_app_version": "5.9.0.68", "hwv": "V1938T", "mobile_no": "", "os_type": "a",
                             "time_str": f"{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}",
                             "user_name": "", "version_no": "5.9.0.8"}}]'''
        '''api = 'com.cars.otsmobile.memberInfo.pointSimpleQuery'
        data = [{"member_id":"","query_type":"0","consume_type":"*",
                 "start_date":"20240328","stop_date":"20250328","point_status":"0",
                 "page_no":"1","page_num":"100","sort_flag":"B",
                 "baseDTO": {"check_code": f"{random_string(32)}",
                             "device_no": f"TEMP-{generate_sign(24)}", "h5_app_id": "60000007",
                             "h5_app_version": "5.9.0.5", "hwv": "V1938T", "mobile_no": "", "os_type": "a",
                             "time_str": f"{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}",
                             "user_name": "", "version_no": "5.9.0.8"}}]'''
        api = 'com.cars.otsmobile.npTicket.queryReserveOrderInfo'
        data = [{"queryType":"4","queryClass":"7","pageIndex":1,"pageSize":10,"queryFlag":"E",
                 "baseDTO": {"check_code": f"{random_string(32)}",
                             "device_no": f"TEMP-{generate_sign(24)}", "h5_app_id": "60000011",
                             "h5_app_version": "5.9.0.11", "hwv": "V1938T", "mobile_no": "", "os_type": "a",
                             "time_str": f"{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}",
                             "user_name": "", "version_no": "5.9.0.8"}}]
...
print(SearchTicket('HBB', 'SHH', '20250401').mobile_search())
