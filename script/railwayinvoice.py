...
def railwayinvoicedata(file_name,check_id,sort_id,input_type,*input_json_data):
    ...
    while True:
        ...
        try:  
            ...
            for data in data_s:
                ...
                for order in data['data']['results']:
...
if __name__ == '__main__':
    ...
    file_name_1 = "railwayinvoice.csv"
    check_id_1 = "ext_ticket_no"
    sort_id_1 = "local_start_time"

    railwayinvoicedata(file_name_1,check_id_1,sort_id_1,1)
...
fetch('https://kyfw.12306.cn/otn/eInvoice/queryPsr', {
    ...
    body: "from_date=10000000&end_date=99999999=&pageIndex=0&ticket_type=&type=1" // 将 URLSearchParams 对象转换为字符串并作为请求体发送
})
.then(response => {
    if (!response.ok) {
...
fetch('https://kyfw.12306.cn/otn/eInvoice/queryPsr', {
    ...
    body: "from_date=10000000&end_date=99999999=&pageIndex=0&ticket_type=&type=2" // 将 URLSearchParams 对象转换为字符串并作为请求体发送
})
.then(response => {
    if (!response.ok) {
        throw new Error('Network response was not ok ' + response.statusText);
