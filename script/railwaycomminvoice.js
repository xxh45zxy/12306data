...
// 第一个 fetch 请求
fetch('https://kyfw.12306.cn/otn/eInvoice/queryNPOrderInvoiceForScanQR', {
    method: 'POST',
    headers: {
        "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8"
    },
    body: "queryType=0&queryClass=1&pageSize=9999&pageIndex=0"
})
.then(response => {
    if (!response.ok) {
...
fetch('https://kyfw.12306.cn/otn/eInvoice/queryNPOrderInvoiceForScanQR', {
    ...
    body: "queryType=0&queryClass=2&pageSize=9999&pageIndex=0"
})
.then(response => {
    if (!response.ok) {
        throw new Error('Network response was not ok ' + response.statusText);
